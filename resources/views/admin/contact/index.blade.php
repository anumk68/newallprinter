                ...
                <table class="table table-striped table-hover" id="contactTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->email }}</td>
                            <td>{{ $item->contact }}</td>
                            <td class="text-wrap">{{ substr($item->subject, 0,32) }}</td>
                            <td class="text-wrap">{{ substr($item->message, 0,32) }}</td>
                            <td>{{ $item->created_at->format('M d, Y')}}</td>
                            <td>
                                <!-- Add action buttons here -->
                                <a href="{{ route('contact.show', $item->id) }}" class="btn btn-info btn-sm">View</a>
                                <form action="{{ route('contact.destroy', $item->id) }}" method="POST" class ="delete-form" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
